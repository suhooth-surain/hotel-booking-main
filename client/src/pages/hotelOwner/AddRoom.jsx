...
const AddRoom = () => {
  ...
  const [inputs, setInputs] = useState({
    roomType: '',
    pricePerNight: 0,
    amenities: { // lowercase 'a' - this is the correct property name
      'Free WiFi': false,
      'Free Breakfast': false, // Fixed typo: Breakfest → Breakfast
      'Room Services': false,
      'Mountain View': false,
      'Pool Access': false,
    }
  ...
  const [loading, setLoading] = useState(false); // Fixed: lowercase and boolean
  ...
  const onSubmitHandler = async (e) => {
    ...
    try {
      ...
      // adding image to form data - FIXED: added parameter to forEach
      Object.keys(images).forEach((key) => {
        images[key] && formData.append('images', images[key])
      })

      // FIXED: await getToken() instead of await getToken
  ...
  return (
    <form onSubmit={onSubmitHandler}>
      ...
      <div className='flex flex-col flex-wrap mt-1 text-gray-400 max-w-sm'>
        {/* FIXED: Changed inputs.Amenities to inputs.amenities */}
      ...
      <button 
        type="submit" 
        className='bg-primary text-white px-8 py-2 rounded mt-8 cursor-pointer disabled:opacity-50' disabled={loading}>

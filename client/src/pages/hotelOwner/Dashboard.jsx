            ...
            {dashboardData.bookings.map((item, index)=>(
              <tr key={index}>
                ...
                <td className='py-3 px-4 border-t border-gray-300 flex'>
                  <button className={`py-1 px-3 text-xs rounded-full max-auto ${item.isPaid ? 'bg-green-200 text-gray-600' : 'bg-amber-200 text-yellow-600'}`}>
                    {item.isPaid ? 'Compleated' : 'Pending'}
                  </button>
                </td>
              </tr>
            ))}
